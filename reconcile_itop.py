...
def get_os_version_id(session, os_version):
    """Query iTop for OS version and return the first match if multiple exist"""
    query = {
        "operation": "core/get",
        "class": "OSVersion",
    ...
    try:
        ...
        if 'objects' in result and result['objects']:
            # Get the first OS version ID from the results
...
def create_itop_machine(fqdn, project, envir, cmsofct, descript, os, os_version, cpu, ram, diskspace, ip, org="CMSO"):
    ...
    # Get OS version ID, using the first match if multiple exist
    os_version_id = get_os_version_id(session, os_version)
    if not os_version_id:
        print(f"Warning: Could not find OS version '{os_version}' in iTop")
        return None
    ...
    machine_data = {
        ...
        "osversion_id": os_version_id,  # Using the resolved OS version ID
...
def main():
    ...
    with open("/tmp/list_itop_machines.csv", "w") as out:
        out.write("FQDN,IP_Adress,OS_FAMILY\n")
